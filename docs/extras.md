...
Vibb has a deliberately tiny extension hook: the owner can drop
launch scripts on the box, and a hidden button chord starts them from
the screen. Nothing else ships in Vibb — no emulators, no extra
...
- **Drop a script in `/etc/vibb/extras/`** (over SSH — this is the
...
- **Hold X + Y (the two right-side buttons) for ~2 seconds** — the
  ...
  nothing.
- Pick an entry, confirm with A. Vibb then stops the screen UI, the
...
- **When your script exits — however it exits — Vibb comes back.**
  The extra runs as a transient systemd unit whose `ExecStopPost`
  restores the full Vibb service set even if the script crashes or
  is killed.
...
- `vibb-daemon` (the HTTP API) is left running on purpose: a linked
...
- You may `systemctl stop` more Vibb services. **Never `disable` or
...
- **Want a word on the box screen when you bail?** Write one line to
  `/run/vibb-extra.msg` before exiting — Vibb's UI shows it for a
  ...
  "no TV found" abort.)
- **The CPU is unparked for you**: Vibb boots in power-save (the
...
`/etc/vibb/extras/retropie.sh` on the box (root-owned, mode 755).
...
- The Pirate Audio display is **not a Linux framebuffer** — RetroArch
  ...
  ST7789 240x240, SPI CE0, DC=BCM9, backlight BCM13). Vibb's UI is
...
- Audio goes out the I2S DAC (ALSA card `sndrpihifiberry` /
  `vibb_local`'s underlying card) — point RetroArch at it.
- 512 MB RAM total: stop `vibb-daemon` too if a core needs the
...
before wiring the script; none of that touches Vibb. `apt install
