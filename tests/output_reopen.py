...
os.environ["VIBB_GO_CONFIG"] = CFG
sys.path.insert(0, os.path.join(REPO, "pi"))

from vibb import output, spotify  # noqa: E402

with open(CFG, "w") as f:
    f.write("bitrate: 320\naudio_device: vibb_local\nvolume: 60\n")

# 1. _write_audio_device rewrites the line in place, once
assert output._write_audio_device("vibb_bt") is True
assert "audio_device: vibb_bt" in open(CFG).read()
assert "bitrate: 320" in open(CFG).read(), "must not clobber other keys"
assert output._write_audio_device("vibb_bt") is False, "unchanged -> no write"
print("1. _write_audio_device: in-place, idempotent, keeps siblings OK")

# 2. missing key -> appended (never silently dropped)
with open(CFG, "w") as f:
    f.write("bitrate: 320\n")
assert output._write_audio_device("vibb_local") is True
assert "audio_device: vibb_local" in open(CFG).read()
...
assert output.reopen_go_output("vibb_bt") is True
assert "audio_device: vibb_bt" in open(CFG).read(), "persist for next boot"
assert CALLS == [("/player/output", {"device": "vibb_bt"})], CALLS
...
spotify.go = _old
assert output.reopen_go_output("vibb_local") is False, "fall back on 404"
...
os.environ.pop("VIBB_GO_CONFIG", None)
